...
field_mapping["classification.identifier"] = "type"
field_mapping["time.source"] = "source time"
...
field_mapping["destination.geolocation.country"] = "dst country"
field_mapping["destination.reverse_dns"] = "dst reverse DNS"
field_mapping["contact"] = "contact"
...
class RtirBot(Bot):
    ...
    def process(self):
        ...
        if not event.contains('rtir.template'):
            self.acknowledge_message()
            self.logger.info("No rtir.template, ignoring: %r", event)
            return

        # Required fields in the event to proceed
        reqfields = ('rtir.classification', 'rtir.subject', 'source.ip',
    ...
    def write_to_rt_ticket(self, event):
        ...
        ticket.cf_severity = event.get("rtir.severity", "Risky")
        ticket.cf_importance = event.get("rtir.importance", "Important")
        ...
        self.logger.info("RT Events written to ticket (#%s)", ticket.ticket_id)
        ...
        if parent_id:
            if ticket_created:
                self.logger.info("RT created parent ticket (#%s)",
                              parent_id)
            else:
                self.logger.info("RT linked parent ticket (#%s)",
                              parent_id)
        else:
        ...
        if logs_missing:
            self.logger.info("Required logs not found for IR %r, only linking "
                             "to incident (#%s)", ticket.ticket_id, parent_id)
            return

        # Created my intelmq
        # FIXME: change this to "intelmq" someday...
        if created_by_ah:
            self.logger.info("Ticket generated by AH, updating incident (#%s)",
                             parent_id)
            ...
            self.logger.info("Incident manually updated "
                             "(AbuseHelperCheck!='Yes'), only linking (#%s)",
                             parent_id)
